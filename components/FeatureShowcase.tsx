          ...
          {chapters.map((c, i) => (
            ...
            >
              <div className={`h-0.5 rounded-full transition-all duration-300 ${
                i === activeChapter ? "w-8 bg-blue-400" : "w-4 bg-slate-600 group-hover:bg-slate-500"
              }`} />
              <span className={`text-xs transition-colors ${
                i === activeChapter ? "text-blue-400 font-semibold" : "text-slate-600 group-hover:text-slate-500"
              }`}>
                {c.label}
              </span>
            </button>
