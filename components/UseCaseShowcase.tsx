...
const SUITES: Suite[] = [
  {
    ...
    useCases: [
      ...
      {
        number: "07",
        problem: "New clients get the same terms as proven ones",
        snapshot: "A new client signs on and gets the same payment terms as a five-year customer with a spotless history. There is no data-driven way to tell the two apart before the first invoice goes out.",
        situation:
          "Extending payment terms is a credit decision, but most service businesses make it without any credit process at all. A new client fills out an intake form, signs a contract, and gets invoiced on the same terms as everyone else. If that client turns out to be a slow payer or a flight risk, the business finds out only after money is already outstanding. Larger companies run credit checks before extending terms. Small service businesses rarely have the tooling to do the same, so every new relationship carries the same invisible risk.",
        context: "The accounts most likely to default or pay slowly are new relationships without payment history, not the long-standing clients who already have a track record. Assessing risk at the start is cheaper than collecting after the fact.",
        fix: [
          ...
          "Risk profile updated automatically as the client builds a payment history with the business",
        ],
        outcomes: [
          { value: "Before", label: "risk is assessed, not after the first missed payment" },
          { value: "Tiered", label: "terms based on risk, not one default for everyone" },
          { value: "Fewer", label: "new accounts turning into write-offs" },
        ],
        accent: "rose",
...

  {
    ...
    useCases: [
      ...
      {
        number: "05",
        problem: "Expense reports are manual, slow, and hard to audit",
        snapshot: "Employees submit receipts by email or spreadsheet weeks after the purchase. Someone checks each line against policy by hand, keys it into the accounting system, and reimburses whenever they get to it.",
        situation:
          "In most small businesses, expense reporting is a manual, ad hoc process. Employees hold on to paper receipts or forward emails, submit them in a batch at the end of the month, and wait for someone to review, correct, and key each line into the accounting system by hand. Policy violations, duplicate submissions, and simple math errors are easy to miss when review happens once a month instead of once per expense. Reimbursement then takes as long as the review queue allows, which is rarely a priority.",
        context: "The cost is not just the reimbursement delay. It is the GL miscoding, the missed policy violations, and the hours spent keying receipts into the accounting system by hand every month.",
        fix: [
          ...
          "Reimbursement scheduled on a regular cycle instead of whenever someone gets to the queue",
        ],
        outcomes: [
          { value: "Same-week", label: "reimbursement instead of end-of-month batches" },
          { value: "Automatic", label: "policy checks on every expense submitted" },
          { value: "Zero", label: "manual GL coding for employee expenses" },
        ],
        accent: "blue",
      ...
      {
        number: "06",
        problem: "Bills get paid without confirming what was ordered or received",
        snapshot: "A vendor bill arrives and gets paid based on the invoice alone. Nobody checks it against the original purchase order or confirms the goods or services were actually received in the quantity billed.",
        situation:
          "Three-way matching, confirming that the purchase order, the vendor invoice, and the delivery or receipt of goods all agree, is standard practice at larger companies specifically because it catches errors that a simple invoice review misses. Most small businesses skip this step entirely, not because it does not matter, but because doing it by hand for every bill is not realistic given everything else on the plate. The result is that overbilling, quantity discrepancies, and duplicate invoices from the same vendor go through unless someone happens to notice.",
        context: "A vendor billing 5 percent over the agreed price on a recurring order is easy to miss one invoice at a time and expensive over a year of invoices.",
        fix: [
          ...
          "Recurring vendors monitored over time for gradual price creep across multiple orders, not just a single invoice",
        ],
        outcomes: [
          { value: "100%", label: "of bills matched against purchase orders before payment" },
          { value: "Caught", label: "overbilling and duplicate charges before they are paid" },
          { value: "Zero", label: "payments released on unconfirmed receipt of goods" },
        ],
        accent: "indigo",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-1 9l-3 3-1.5-1.5" /></svg>,
      },
    ],
  },

  {
    ...
    useCases: [
      ...
      {
        number: "04",
        problem: "Payroll runs on manual timesheets and manual tax rules",
        snapshot: "Hours get tracked on paper or in a spreadsheet, someone converts them into a payroll run by hand, and multi-state tax withholding is calculated based on whatever the last person figured out.",
        situation:
          "Payroll looks simple from the outside: pay people the right amount on time. In practice it means reconciling hours from time tracking, applying the correct withholding for every state an employee works in, keeping up with changing tax tables, and generating 1099s correctly for contractors at year end. Doing this by hand works until it does not, usually when a state tax rule changes or an employee moves, and by then the error may have compounded across several pay periods.",
        context: "The businesses most exposed here are the ones with contractors or employees in more than one state, where the withholding rules are the most likely to be applied incorrectly by hand.",
        fix: [
          ...
          "Payroll cost data synced to the general ledger automatically, coded correctly by department or project",
        ],
        outcomes: [
          { value: "Zero", label: "manual timesheet-to-payroll re-entry" },
          { value: "Automatic", label: "multi-state withholding, updated as rules change" },
          { value: "Ready", label: "1099s at year end, not a January scramble" },
        ],
        accent: "cyan",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M7 20H2v-2a3 3 0 015.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>,
      },
      {
        number: "05",
        problem: "The books are never quite in sync with the bank",
        snapshot: "Transactions get categorized whenever someone has time, usually right before month end. Until then, nobody can say with confidence what the actual cash position is on any given day.",
        situation:
          "Bank and credit card reconciliation is usually treated as a month-end task, which means the accounting system is stale for most of the month. A payment clears, a card charge posts, and neither is categorized or matched to the right transaction until the person doing the books catches up, sometimes weeks later. In the meantime, decisions about cash on hand are made on a bank balance that does not reflect pending items, uncategorized charges, or transactions that have not been recorded yet.",
        context: "A business making decisions off a bank balance instead of a reconciled ledger is making decisions with a blind spot that grows every day between reconciliations.",
        fix: [
          ...
          "A reconciled, current cash position available every day instead of only after month end",
        ],
        outcomes: [
          { value: "Daily", label: "reconciliation instead of a month-end scramble" },
          { value: "Same-day", label: "discrepancy flags instead of weeks-later surprises" },
          { value: "Current", label: "cash position, available any day of the month" },
        ],
        accent: "orange",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M3 21h18M5 21V10m4 11V10m4 11V10m4 11V10m4 11V10M3 10l9-6 9 6H3z" /></svg>,
      },
      {
        number: "06",
        problem: "Sales tax is calculated by guesswork and filed manually",
        snapshot: "The business collects sales tax using a rate that has not been checked in years, in whatever states someone remembers to register in. Filing happens manually, state by state, close to each deadline.",
        situation:
          "Sales tax obligations follow the customer, not the business, and the rules for when a business owes tax in a state, known as nexus, have gotten more complicated as remote work and multi-state clients have become normal. Most small businesses are not deliberately noncompliant. They simply do not have a system for tracking which states they have crossed a threshold in, what the current rate is in each one, and when each state's filing is due. The risk sits quietly until an audit or a state notice surfaces it.",
        context: "Sales tax penalties and back taxes are one of the more common surprises for growing service businesses, precisely because the obligation can exist in a state long before anyone notices it.",
        fix: [
          ...
          "A complete audit trail of what was collected, filed, and remitted, ready if a state ever asks",
        ],
        outcomes: [
          { value: "Tracked", label: "nexus exposure across every state automatically" },
          { value: "Current", label: "tax rates applied at the time of invoicing" },
          { value: "On time", label: "filings across every registered state" },
        ],
        accent: "yellow",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M9 14l6-6M9.5 9h.01M14.5 14h.01M7 3h10a1 1 0 011 1v16l-2-1.5-2 1.5-2-1.5-2 1.5-2-1.5V4a1 1 0 011-1z" /></svg>,
      },
      {
        number: "07",
        problem: "Unusual transactions are only caught by accident",
        snapshot: "A duplicate payment, a slightly altered vendor invoice, or a transaction that breaks an otherwise consistent pattern usually only gets caught if someone happens to notice while doing something else.",
        situation:
          "Fraud and simple errors both tend to look the same at first: a transaction that does not match the pattern of everything around it. A duplicate vendor payment, an invoice that arrives twice with slightly different formatting, or a new payment method for a long-standing vendor are all the kind of thing a careful person might catch, if they happen to be looking at the right transaction at the right time. Most small businesses do not have anyone whose job is to look for this full time, so it is caught by luck more often than by process.",
        context: "Detecting an issue at the transaction level, before money moves, is a fundamentally different outcome than discovering it during a reconciliation weeks later.",
        fix: [
          ...
          "A running audit trail of flagged items and their resolution, useful for both controls and year-end review",
        ],
        outcomes: [
          { value: "Before", label: "payment goes out, not after it is discovered missing" },
          { value: "Prioritized", label: "by dollar impact, not a flat list of alerts" },
          { value: "Fewer", label: "duplicate payments and unnoticed anomalies" },
        ],
        accent: "rose",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-4.35-4.35m0 0a7.5 7.5 0 10-1.06 1.06l1.06-1.06zM10.5 7.5v3m0 3h.01" /></svg>,
      },
      {
        number: "08",
        problem: "Nobody notices a budget is blown until it already happened",
        snapshot: "A budget gets built once a year and rarely looked at again. Spending in any category can run well over plan for months before anyone outside the bookkeeper notices.",
        situation:
          "Building a budget is common. Actually tracking performance against it every month is much less common, because it requires pulling actuals, comparing them category by category, and flagging what is off, which is exactly the kind of recurring manual task that gets skipped when things get busy. The budget ends up being a planning exercise rather than a management tool, and variances are only noticed well after the spending already happened.",
        context: "A variance caught in month one is a conversation. The same variance caught in month nine is a pattern that has already cost real money.",
        fix: [
          ...
          "Budget owners notified directly when their category moves outside an agreed range",
        ],
        outcomes: [
          { value: "Monthly", label: "budget versus actual, not an annual after-the-fact review" },
          { value: "Early", label: "flags on variances, before they compound over months" },
          { value: "Direct", label: "notification to the budget owner, not a buried report" },
        ],
        accent: "teal",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M8 21V10m4 11V6m4 15v-9m4 9V3" /></svg>,
      },
      {
        number: "09",
        problem: "Depreciation schedules are a spreadsheet nobody trusts",
        snapshot: "Equipment and other fixed assets get tracked, if at all, in a spreadsheet built years ago. Depreciation is calculated by hand each year, and assets that were sold or retired are not always removed.",
        situation:
          "Every fixed asset a business owns needs to be depreciated on a schedule, and disposed of assets need to come off the books. In practice, this usually lives in a spreadsheet maintained by whoever set it up originally, updated inconsistently, and trusted by nobody including the person maintaining it. When an asset is sold, scrapped, or replaced, the update to the depreciation schedule and the balance sheet is easy to forget. The result is a fixed asset register that technically exists but does not reflect what the business actually owns.",
        context: "An inaccurate fixed asset register does not just affect the balance sheet. It affects insurance coverage, tax depreciation, and any due diligence process if the business is ever sold or refinanced.",
        fix: [
          ...
          "A current, accurate fixed asset register available at any time, not rebuilt from memory when someone asks for it",
        ],
        outcomes: [
          { value: "Automatic", label: "depreciation posted every period, not once a year by hand" },
          { value: "Current", label: "fixed asset register, updated as assets change" },
          { value: "Accurate", label: "balance sheet asset values, ready for any review" },
        ],
        accent: "indigo",
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M20 7L12 3 4 7m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>,
      },
    ],
  },
];
